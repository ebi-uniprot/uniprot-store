...
- JaCoCo is enabled through the Maven lifecycle and the aggregate report is generated by
  the `jacoco-aggregate-report` module during `verify`.
...
Some integration tests start local Spark, Solr, ZooKeeper, or Voldemort-related
services and bind local ports. Run those tests in an environment that allows local
socket binding.
